Meteor.voting = {
  ...
  voteArticleUp: function(event, articleId) {
    if (Meteor.user()) {
      ...
      //Undo upvote!
      if ($.inArray(Meteor.userId(), article.upVoted) !== -1) {
        Articles.update(selectedId, { $pull: {
            upVoted: Meteor.userId()
          },
          $inc: { 'score': -1 }
        });
      ...
      Articles.update(selectedId, update);
    }
  },
  voteArticleDown: function(event, articleId) {
    if (Meteor.user()) {
      var article = Articles.findOne({ _id: articleId }),
          update = {},
          netUpdateScore = 0,
      ...
      if ($.inArray(Meteor.userId(), article.downVoted) !== -1) {
        Articles.update(selectedId, { $pull: {
            downVoted: Meteor.userId()
          },
          $inc: { 'score': 1 }
      ...
      Article.update(selectedId, update);

      if (post.score <= -100 && !post.adminPost) {
